...
SELECT DISTINCT n.*, a.turn_out
FROM na_lookup n
LEFT JOIN elections_v2 a
ON n.na_value = CAST(a.na_value as int) AND a.year = 2013
ORDER BY na_value
LIMIT 300;
...
SELECT * FROM na_lookup
WHERE na_value NOT IN (SELECT DISTINCT CAST(na_value as int) FROM elections_v2 WHERE year = 2013);

SELECT a.*, b.party
FROM na_lookup a
LEFT OUTER JOIN elections_v2 b
ON a.na_value = CAST(b.na_value as int)
AND b.year = 2013 and b.rvotes = 1
;
