...
---------------------------------------------------------------------
/* Get similar victory_margin metrics when comparing 1997 to 2002 */
---------------------------------------------------------------------


-- Table comparing 2008 and 2013 results. Goal is to identify which seats were retained signnified by retention_flag = 1
CREATE TEMP TABLE comp_1997_2002 AS
SELECT a.*, b.year as next_year, b.na_name as next_na_name, b.winner as next_winner, b.winner_party as next_winner_party, b.victory_margin as next_victory_margin, 
b.winner_party_num as next_winner_party_num, 0 as retention_flag
FROM metadata_1997 a
INNER JOIN metadata_2002 b
ON a.na_number = b.na_number
;
-- 206 rows
...
--From seats where winner_party_nums were different, look at seats belonging to PML-N
SELECT winner_party, winner_party_num, next_winner_party, next_winner_party_num, count(*) as seats 
FROM comp_1997_2002
WHERE winner_party_num != next_winner_party_num
AND winner_party_num = 121
GROUP BY winner_party, winner_party_num, next_winner_party, next_winner_party_num;

--From seats where winner_party_nums were different, look at seats belonging to HPG
SELECT winner_party, winner_party_num, next_winner_party, next_winner_party_num, count(*) as seats 
FROM comp_1997_2002
WHERE winner_party_num != next_winner_party_num
AND winner_party_num = 30
GROUP BY winner_party, winner_party_num, next_winner_party, next_winner_party_num;
...
---------------------------------------------------------------------
/* Get similar victory_margin metrics when comparing 1993 to 1997 */
---------------------------------------------------------------------


-- Table comparing 1993 and 1997 results. Goal is to identify which seats were retained signnified by retention_flag = 1
CREATE TEMP TABLE comp_1993_1997 AS
SELECT a.*, b.year as next_year, b.na_name as next_na_name, b.winner as next_winner, b.winner_party as next_winner_party, b.victory_margin as next_victory_margin, 
b.winner_party_num as next_winner_party_num, 0 as retention_flag
FROM metadata_1993 a
INNER JOIN metadata_1997 b
ON a.na_number = b.na_number
;
-- 205 rows
...
--From seats where winner_party_nums were different, look at seats belonging to PPP
SELECT winner_party, winner_party_num, next_winner_party, next_winner_party_num, count(*) as seats 
FROM comp_1993_1997
WHERE winner_party_num != next_winner_party_num
AND winner_party_num = 124
...
WHERE retention_flag = 1
;
-- avg victory_margin = 15.7

SELECT * FROM comp_1993_1997
WHERE retention_flag = 1
ORDER BY victory_margin
;
-- median victory margin = 11.9
